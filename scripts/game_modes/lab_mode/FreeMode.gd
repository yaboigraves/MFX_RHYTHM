...
func enter(_msg := {}) -> void:
	...
	metronome._on_player_beat_phase_callback(currentGameState, HandleRecordStateEnd, true)

func HandleRecordStateEnd():
	player.StartVerifying(currentGameState)
	metronome._on_player_beat_phase_callback(currentGameState, HandleVerifyStateEnd, true)
